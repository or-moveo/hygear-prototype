...
function stepRadius(i, total) {
  const bl = i === 0 ? 24 : 6
  const br = i === total - 1 ? 24 : 6
  return `24px 24px ${br}px ${bl}px`
}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
          <div className="flex items-center gap-4">
            <div className="px-3 py-1.5 rounded-lg font-poppins font-bold text-white text-sm" style={{ backgroundColor: cls.color }}>
              {cls.time}
            </div>
            <div>
        ...
        <div className="px-8 py-6 flex flex-col gap-6">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(79.86deg, #435a97 0%, #6685cd 100%)' }}
          >
...
export default function BODashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#f2f2f5] font-poppins">
      <StudioHeader name="Backoffice Studio" variant="fluid" />

      {/* Hero summary banner */}
      <div
        className="mx-6 mt-6 flex items-center justify-between px-8 py-5"
        style={{ background: 'linear-gradient(79.86deg, #435a97 0%, #6685cd 100%)', borderRadius: 24 }}
      >
        <div className="flex flex-col gap-1">
          <p className="font-poppins font-normal text-sm text-white/60 uppercase tracking-widest">Today's Schedule</p>
          <p className="font-poppins font-bold text-2xl text-white">{todayClasses.length} Classes</p>
        </div>
      ...
      <div className="px-6 py-6 flex gap-6">
        ...
        <div className="flex-1">
          <h2 className="font-poppins font-bold text-xl text-[#334367] mb-4">Today's Classes</h2>
          <div className="flex flex-col gap-4">
            {todayClasses.map(cls => (
              <div
                key={cls.id}
                onClick={() => setSelectedClass(cls)}
                className="flex items-center gap-4 cursor-pointer hover:-translate-y-0.5 transition-all duration-200"
                style={{
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: cls.color, borderRadius: 999 }}
                >
                  {cls.time}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-poppins font-semibold text-[#334367] truncate">{cls.name}</p>
                  <p className="text-sm text-gray-500">{cls.coach} &middot; {cls.room}</p>
                </div>
                <div className="shrink-0 flex items-center gap-1.5 text-sm text-gray-600">
                  <Users size={16} />
                  <span className="font-semibold">{cls.enrolled}</span>
                  <span className="text-gray-400">/ {cls.capacity}</span>
                </div>
                <span className="shrink-0 px-3 py-1 rounded-full text-xs font-semibold bg-[#edf3ef] text-[#334367]">
                  {cls.type}
                </span>
              </div>
            ))}
        ...
        {/* Right: Stats + Alerts + Quick Actions */}
        <div className="w-[320px] shrink-0 flex flex-col gap-4">
          ...
          <div className="p-6" style={{ borderRadius: 24, background: '#fff' }}>
            <h3 className="font-poppins font-semibold text-[#334367] text-sm mb-3">Active Alerts</h3>
            <div className="flex flex-col gap-2">
              {alerts.map(alert => {
                ...
                return (
                  <div
                    key={alert.id}
                    className="flex items-start gap-2.5 p-2.5 text-sm"
          ...
          <div className="p-6" style={{ borderRadius: 24, background: '#fff' }}>
            <h3 className="font-poppins font-semibold text-[#334367] text-sm mb-3">Quick Actions</h3>
