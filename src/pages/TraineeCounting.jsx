...
/**
 * Trainee version of the "Counting" countdown screen
 * (the original Studio screen lives in `Counting.jsx`).
 *
 * Same content (cyan glass card with a pop-animated 3 → 2 → 1 digit), scaled to fit
 * the 1366×1024 Trainee frame. Per the established Trainee rules:
 *
 ...
 *     The inner digit box stays at its original size and is centered, so the count-pop
 *     animation runs identically — only spacing around it grows.
 */
...
// Per-digit box widths so each glyph sits with consistent visual padding (Studio parity).
const DIGIT_BOX = {
  1: 245,
  2: 297,
  3: 302,
...
export default function TraineeCounting() {
  ...
  const CARD_HEIGHT = 1190 // Outer card: 888 → 1190 to fill the taller Trainee frame.
      ...
      <div style={{ position: 'relative', width: 1366, height: 1024, overflow: 'hidden' }}>
        <StageBackground variant="dark" glowColor={CYAN} showTopbar={false}>
          ...
          <div style={{
            ...
            >
              {/* Inner black box — width adapts per digit, kept at original size & animation. */}
              <div
                key={digit}
                style={{
