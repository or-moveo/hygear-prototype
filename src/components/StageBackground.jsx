...
/**
 * @param {{ studioName?: string, showTopbar?: boolean, children?: React.ReactNode }} props
 */
export default function StageBackground({ studioName = 'Studio name', showTopbar = true, children }) {
  return (
    <div style={{ width: 1920, height: 1080, background: '#000', fontFamily: 'Poppins, sans-serif', overflow: 'hidden', position: 'relative' }}>
      ...
      {/* Aurora blobs */}
      <div style={{ position:'absolute', width:900, height:900, borderRadius:'50%', background:'#FF6B00', left:-200, top:200, filter:'blur(120px)', opacity:.22, mixBlendMode:'screen', animation:'sbg-drift1 22s ease-in-out infinite', pointerEvents:'none', zIndex:0 }} />
      <div style={{ position:'absolute', width:900, height:900, borderRadius:'50%', background:'#3A86FF', right:-200, top:-150, filter:'blur(120px)', opacity:.22, mixBlendMode:'screen', animation:'sbg-drift2 28s ease-in-out infinite', pointerEvents:'none', zIndex:0 }} />
      <div style={{ position:'absolute', width:900, height:900, borderRadius:'50%', background:'#F5365C', left:'40%', bottom:-400, filter:'blur(120px)', opacity:.14, mixBlendMode:'screen', animation:'sbg-drift3 30s ease-in-out infinite', pointerEvents:'none', zIndex:0 }} />

      {/* Sparkle haloes */}
      <svg style={{ position:'absolute', width:661, height:661, left:0, top:749, color:'#fff', opacity:.25, filter:'blur(90px) drop-shadow(0 0 80px rgba(255,255,255,.3))', animation:'sbg-floatBL 22s ease-in-out infinite, sbg-spinSlow 80s linear infinite reverse, sbg-glowPulse 6s ease-in-out infinite', mixBlendMode:'screen', pointerEvents:'none', zIndex:0 }} viewBox="0 0 661.125 661.125" aria-hidden="true">
        <path d={SPARKLE_PATH} fill="currentColor"/>
      </svg>
      <svg style={{ position:'absolute', width:661, height:661, left:1259, top:-331, color:'#fff', opacity:.25, filter:'blur(90px) drop-shadow(0 0 80px rgba(255,255,255,.3))', animation:'sbg-floatTR 18s ease-in-out infinite, sbg-spinSlow 60s linear infinite, sbg-glowPulse 5s ease-in-out infinite', mixBlendMode:'screen', pointerEvents:'none', zIndex:0 }} viewBox="0 0 661.125 661.125" aria-hidden="true">
        <path d={SPARKLE_PATH} fill="currentColor"/>
      </svg>
      ...
      {/* Energy rail — animated white glow gliding left→right across the full width */}
      <div className="sbg-rail" style={{
        ...
        opacity:.2, mixBlendMode:'screen', zIndex:1,
      }} />
      ...
      {showTopbar && (
        <div style={{
          ...
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            <HygearLogo />
            <span style={{ fontWeight:700, fontSize:36, lineHeight:'46px', color:'#fff' }}>{studioName}</span>
          </div>
        </div>
      )}
